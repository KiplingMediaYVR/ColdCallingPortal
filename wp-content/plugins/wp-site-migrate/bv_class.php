...
class BlogVault {
	...
	*/
	...
	function getWpInfo() {
		...
		global $wpdb, $bvVersion, $wp_local_package;
		$wp_info = array(
	...
	function getAsymKey() {
		...
		if (preg_match('/^\w+$/', $keyName)) {
			...
			$this->addStatus('asymerror', 'badkey');
			return false;
		}
	}
	...
	function asymEncrypt($source) {
		if (function_exists('openssl_public_encrypt')) {
			...
			$this->addStatus('asymerror', 'openssl_public_encrypt');
			return false;
		}
	}
	...
	function asymAuth() {
		...
		if (function_exists('openssl_public_decrypt')) {
			...
			$this->addStatus('asymerror', 'mismatch');
		} else {
			$this->addStatus('asymerror', 'openssl_public_decrypt');
		}
		return false;
	}
	...
	function processManageCallback($method) {
		global $bvmanager;
		if (!class_exists('BVManageSite')) {
	...
	function processCallbackMethod($method) {
		switch ($method) {
		case "sendmanyfiles":
	...
	function processApiRequest() {
		...
		if (array_key_exists('asymauth', $_REQUEST)) {
			if ($this->asymAuth() === 1) {
		...
		} else if ($this->authenticateControlRequest() === 1) {
			$this->processParameters();
			$method = urldecode($_REQUEST['bvMethod']);
			$this->addStatus("callback", $method);
			if (array_key_exists('manage', $_REQUEST) || array_key_exists('mng', $_REQUEST)) {
				$this->processManageCallback($method);
			} else {
				$this->processCallbackMethod($method);
			}
			$this->terminate(true);
		} else {
			$this->addStatus("statusmsg", 'failed authentication');
