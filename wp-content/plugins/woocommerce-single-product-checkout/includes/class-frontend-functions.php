...
class WooCommerce_Single_Product_Checkout_Frontend extends WooCommerce_Single_Product_Checkout {
	...
	public function __construct() {
        ...
        $this->is_loop_update = '';
        add_filter('woocommerce_add_to_cart_validation', array($this,'check_cart'),99,3);
        add_action('woocommerce_after_cart_item_quantity_update',array($this,'cart_qty_update'),99,3);
        add_filter('woocommerce_add_to_cart_redirect',array($this,'change_redirect_url'),999);
    }
    ...
    public function cart_qty_update($cart_item_key, $quantity, $old_quantity ){
        ...
        if(!empty($status)){ 
            ...
            if(($quantity >= $min_qty) === false){
                $message = get_option(WC_SPC_DBKEY.'product_with_min_qty_error');
            ...
            if($quantity <= $max_qty ){
                ...
                $message = get_option(WC_SPC_DBKEY.'product_with_qty_error');
            ...
            if(! empty($productids)){
                foreach($productids as $id){
                    ...
                    if(!empty($status)){
                        ...
                        wc_add_notice(get_option(WC_SPC_DBKEY.'single_product_other_error'),'error');
                        return false;
                    } else {
                        continue;
                    }
    ...
    public function check_cart_contents($product_id){
        ...
        if(! empty($productids)){
            if(in_array($product_id ,$productids)){
                ...
                wc_add_notice(get_option(WC_SPC_DBKEY.'other_product_error'),'error');
                return false;
            }
        } 
        return true;
    ...
    public function change_redirect_url($url){ 
        if(isset($_REQUEST['add-to-cart'])){
            $status = $this->get_product_status($_REQUEST['add-to-cart']);
            if(!$status){
                return $url;
            }
        }
        
        $redirect_to = get_option(WC_SPC_DBKEY.'redirect_to');
