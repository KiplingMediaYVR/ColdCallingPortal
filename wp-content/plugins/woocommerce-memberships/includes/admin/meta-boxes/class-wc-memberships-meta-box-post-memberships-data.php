...
class WC_Memberships_Meta_Box_Post_Memberships_Data extends WC_Memberships_Meta_Box {
	...
	public function output( WP_Post $post ) {
		...
		<div class="js-restrictions <?php if ( 'yes' === wc_memberships_get_content_meta( $post, '_wc_memberships_force_public', true ) ) : ?>hide<?php endif; ?>">
			...
			?>
			<div
				class="js-custom-message-editor-container <?php if ( $use_custom !== 'yes' ) : ?>hide<?php endif; ?>"
	 ...
	 * @return string either 'yes', 'no' or false if meta is not set
	 */
	private function get_use_custom_message_flag( $post_id, $message_code ) {
		...
		$legacy_flag                    = wc_memberships_get_content_meta( $post_id, $use_custom_message_legacy_meta, true );

		return ! empty( $legacy_flag ) ? $legacy_flag : wc_memberships_get_content_meta( $post_id, $use_custom_message_meta, true );
	}
	...
	private function get_custom_message( $post_id, $message_code ) {
		...
		$legacy_message      = wc_memberships_get_content_meta( $post_id, $message_legacy_meta, true );

		return ! empty( $legacy_message ) ? $legacy_message : wc_memberships_get_content_meta( $post_id, $message_meta, true );
	}
