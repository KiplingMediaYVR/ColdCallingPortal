...
class WC_Memberships_Admin {
	...
	public function get_screen_ids( $context = null ) {
		...
		$screen_ids = (array) apply_filters( 'wc_memberships_admin_screen_ids', array(
			...
			'scripts'    => array_merge( $tabs_screens, $scripts_screens, $meta_boxes_screens ),
			'tabs'       => $tabs_screens,
		) );
	...
	private function init_modals() {
		...
		if ( $screen = get_current_screen() ) {
			...
			if ( $screen->id       === 'edit-wc_user_membership' ) {
				$modals_classes[] = 'WC_Memberships_Modal_Add_User_Membership';
			} elseif ( $screen->id === 'wc_user_membership' ) {
				$modals_classes[] = 'WC_Memberships_Modal_Add_User_Membership';
	...
	private function init_meta_boxes() {
		...
		// load and instantiate objects
		$meta_boxes = $this->init_objects( 'WC_Memberships_Meta_Box', $meta_box_classes, '/includes/admin/meta-boxes/' );
	...
	private function enqueue_scripts() {
		...
		$jquery   = array( 'jquery' );
		$backbone = array( 'jquery', 'backbone', 'wc-backbone-modal' );

		// base scripts
		wp_register_script( 'wc-memberships-enhanced-select',     $path . 'wc-memberships-enhanced-select.min.js', array_merge( $jquery, array( 'select2' ) ), $ver );
		wp_register_script( 'wc-memberships-rules',               $path . 'wc-memberships-rules.min.js',           array( 'wc-memberships-enhanced-select' ), $ver );
		wp_register_script( 'wc-memberships-modal',               $path . 'wc-memberships-modal.min.js',           $backbone, $ver );
		wp_register_script( 'wc-memberships-modals',              $path . 'wc-memberships-modals.min.js',          array( 'wc-memberships-modal', 'wc-memberships-enhanced-select' ), $ver );
		wp_enqueue_script(  'wc-memberships-admin',               $path . 'wc-memberships-admin.min.js',           array( 'wc-memberships-enhanced-select', 'wc-memberships-rules', 'wc-memberships-modals' ), $ver );
		...
		if ( $screen && in_array( $screen->id, array( 'wc_membership_plan', 'edit-wc_membership_plan' ), false ) ) {
			wp_enqueue_script( 'wc-memberships-membership-plans', $path . 'wc-memberships-plans.min.js',            array_merge( $jquery, array( 'wc-memberships-admin', 'jquery-ui-datepicker' ) ), $ver );
		} elseif ( $screen && in_array( $screen->id, array( 'wc_user_membership', 'edit-wc_user_membership' ), false ) ) {
			wp_enqueue_script( 'wc-memberships-user-memberships', $path . 'wc-memberships-user-memberships.min.js', array_merge( $jquery, array( 'wc-memberships-admin', 'jquery-ui-datepicker', 'wc-memberships-modals' ) ), $ver );
		} elseif ( $this->is_memberships_import_export_admin_screen( $screen ) ) {
			wp_enqueue_script( 'wc-memberships-import-export',    $path . 'wc-memberships-import-export.min.js',    array_merge( $jquery, array( 'jquery-ui-datepicker' ) ), $ver );
		} elseif ( wc_memberships()->is_plugin_settings() ) {
			wp_enqueue_script( 'wc-memberships-settings',         $path . 'wc-memberships-settings.min.js',         array_merge( $jquery, array( 'wc-memberships-admin' ) ), $ver );
		}
	...
	/**
	 * Processes import / export page input form.
	 *
	 * @since 1.6.0
	 */
	public function process_import_export_form() {
		...
		/**
		 * Fires upon Memberships importing or exporting.
		 *
		 * @since 1.6.0
		 *
	...
	public function __call( $method, $args ) {
		...
		switch ( $method ) {
			...
			/** @deprecated since 1.7.4 - remove by 1.10.0 or higher */
			case 'add_user_columns' :
				_deprecated_function( $deprecated, '1.7.4', 'wc_memberships()->get_admin_instance()->get_users_instance()->add_user_columns()' );
			...
			/** @deprecated since 1.7.4 - remove by 1.10.0 or higher */
			case 'duplicate_product_memberships_data' :
				_deprecated_function( $deprecated, '1.9.0' );
				$new_product = isset( $args[0] ) ? $args[0] : null;
				$old_product = isset( $args[1] ) ? $args[1] : null;
				wc_memberships()->get_admin_instance()->get_products_instance()->duplicate_product_memberships_data( $new_product, $old_product );
				return null;
			...
			case 'show_user_memberships' :
				_deprecated_function( $deprecated, '1.7.4', 'wc_memberships()->get_admin_instance()->get_users_instance()->show_user_memberships()' );
			...
			case 'user_column_values' :
				_deprecated_function( $deprecated, '1.7.4', 'wc_memberships()->get_admin_instance()->get_users_instance()->user_column_values()' );
